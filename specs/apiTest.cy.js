...
describe("Public API tests", () => {
  it("Checking public API", () => {
    cy.request('GET', Cypress.config("apiUrl")+'/info')
        .then((response) => {
...
describe("Non-Public API tests", () => {
  it("Checking non-public API", () => {
    ...
    for(let i=0; i<=nameValues.length-1; i++){
        cy.request({
            method: 'GET',
            url: Cypress.config("apiUrl")+'/search/masterCostItems?'+nameValues[i]+'=test&token=123&serviceId=web/edge&serviceVersion=1.5.18.2',
  ...
  it("Checking non-public API 2", () => {
        cy.request({
            method: 'GET',
            url: Cypress.config("apiUrl")+'/search/masterCostItems',
            failOnStatusCode: false
          }).then((response) => {
